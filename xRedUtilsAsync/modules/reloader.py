"""
This module provides async functions for `"hot reloading"`.
...
async def load_module(path: str) -> types.ModuleType:
    """
...
async def reload_module(path: str, ignore_not_loaded: bool = False) -> types.ModuleType | None:
    """
    ...
    """
    if not await unload_module(path) and not ignore_not_loaded:
